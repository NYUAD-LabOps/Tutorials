...
TX_THREAD EthernetPrimary;
void EthernetPrimary_create(void);
static void EthernetPrimary_func(ULONG thread_input);
static uint8_t EthernetPrimary_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.EthernetPrimary") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
void EthernetPrimary_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&EthernetPrimary, (CHAR *) "EthernetPrimary", EthernetPrimary_func, (ULONG) NULL,
                            &EthernetPrimary_stack, 1024, 2, 2, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&EthernetPrimary, 0);
    }
}
